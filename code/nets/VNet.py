...
class Encoder(nn.Module):
    ...
    def forward(self, input, pcaW=None):
        x1 = self.block_one(input)
        if pcaW is not None:
            x1 = self.reconstruct_and_classify(x1, pcaW) + x1
    ...
    def reconstruct_and_classify(self, features, W):
        if len(features.shape)==4:
            ...
            b, c, h, w, z = features.size()
            B_features = features.permute(0, 2, 3, 4, 1).flatten(0, 3)
        ...
        if len(features.shape)==4:
            ...
            reconstructed_features = reconstructed_features.view(c, b, h, w, z).permute(1, 0, 2, 3, 4)
...
class VNet(nn.Module):
    ...
    def forward(self, input, pcaW=None):
        ...
        features = self.encoder(input, pcaW)
