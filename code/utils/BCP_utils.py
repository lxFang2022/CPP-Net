...
import random
from utils.losses import mask_DiceLoss, MaskAMOSDiceLoss
from scipy.ndimage import distance_transform_edt as distance
from skimage import segmentation as skimage_seg

# DICE = mask_DiceLoss(nclass=16)
DICE = MaskAMOSDiceLoss(nclass=16).cuda()
...
# def multi_mix_loss(net3_output, img_l, patch_l, mask, l_weight=1.0, u_weight=0.5, unlab=False):
#     img_l, patch_l = img_l.type(torch.int64), patch_l.type(torch.int64)
#     image_weight, patch_weight = l_weight, u_weight
#     if unlab:
#         image_weight, patch_weight = u_weight, l_weight
...
#     loss_ce = image_weight * (CE(net3_output, img_l) * mask).sum() / (mask.sum() + 1e-16)
#     loss_ce += patch_weight * (CE(net3_output, patch_l) * patch_mask).sum() / (patch_mask.sum() + 1e-16)
#     return dice_loss, loss_ce


def multi_mix_loss(
    net3_output,            # [B, C, ...] logits（若 DICE 需要概率，见下）
    img_l,                  # [B, ...] long
    patch_l,                # [B, ...] long
    mask,                   # [B, ...] 或 [B,1,...]，0/1
    l_weight: float = 1.0,
    ...
    unlab: bool = False,
    eps: float = 1e-12,
    ce: nn.Module = None,   # 可传入已有 CE/FocalLoss；不传则内部创建 CE reduction='none'
    ce_loss: nn.Module = None,  # alias for ce, takes precedence
    dice_from_logits: bool = True,  # 若你的 DICE 期望概率，则设 False
    dice_loss: nn.Module = None,    # 可传入带类别权重的 Dice loss；不传则使用全局 DICE
):
    # dtypes
    img_l   = img_l.long()
    ...
    # 统一 mask 形状/类型
    if mask.dim() == net3_output.dim() and mask.size(1) == 1:
        mask = mask.squeeze(1)
    mask = mask.float()
    patch_mask = 1.0 - mask  # 假设原始 mask ∈ {0,1}
    ...
    # ----- Dice -----
    _dice = dice_loss if dice_loss is not None else DICE
    if dice_from_logits:
        dice_img   = _dice(net3_output, img_l,   mask)        # scalar
        dice_patch = _dice(net3_output, patch_l, patch_mask)  # scalar
    else:
        probs = F.softmax(net3_output, dim=1)
        dice_img   = _dice(probs, img_l,   mask)
        dice_patch = _dice(probs, patch_l, patch_mask)

    dice_loss = image_weight * dice_img + patch_weight * dice_patch

    # ----- Cross-Entropy / Focal Loss（逐元素加权）-----
    _ce = ce_loss if ce_loss is not None else (ce if ce is not None else nn.CrossEntropyLoss(reduction='none'))

    ce_img   = _ce(net3_output, img_l)     # [B, ...]
    ce_patch = _ce(net3_output, patch_l)   # [B, ...]

    img_den   = mask.sum().clamp_min(eps)
    patch_den = patch_mask.sum().clamp_min(eps)

    loss_ce = image_weight * (ce_img * mask).sum()       / img_den \
            + patch_weight * (ce_patch * patch_mask).sum() / patch_den
