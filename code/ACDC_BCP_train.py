...
parser.add_argument('--exp', type=str, default='CPP', help='experiment_name')
parser.add_argument('--model', type=str, default='unet', help='model_name')
parser.add_argument('--pre_iterations', type=int, default=10000, help='maximum epoch number to train') #100
parser.add_argument('--max_iterations', type=int, default=30000, help='maximum epoch number to train')
parser.add_argument('--batch_size', type=int, default=24, help='batch_size per gpu')
parser.add_argument('--deterministic', type=int,  default=1, help='whether use deterministic training')
parser.add_argument('--base_lr', type=float,  default=0.01, help='segmentation network learning rate')
parser.add_argument('--mode', type=str,  default='no', help='segmentation network learning rate mode')
...
parser.add_argument('--rate', type=float,  default=1/2, help='multinum of random masks')
#flx
...
parser.add_argument('--pre_epoch', type=int, default=50, help='pre_epoch')
parser.add_argument('--self_epoch', type=int, default=150, help='self_epoch')
parser.add_argument('--pca_weight', type=int, default=0.01, help='pca_weight')
...
def generate_mask(img):
    ...
    patch_x, patch_y = int(img_x*rate), int(img_y*rate)
...
def mix_loss(output, img_l, patch_l, mask, l_weight=1.0, u_weight=0.5, unlab=False, mode=0):
    if args.weightedbce:
    ...
    if mode == 1 and args.weightedbce:
        weights = losses.bce_weight2(output_soft,args.num_classes)
        ...
        loss_ce += patch_weight * (CE(output, patch_l, weights) * patch_mask).sum() / (
...
def mix_loss2(output, img_l, patch_l, mask, mode=0):
    if args.weightedbce:
        CE = losses.WeightedCrossEntropyLoss()
    else:
        CE = nn.CrossEntropyLoss(reduction='none')
    ...
    if mode == 1 and args.weightedbce:
        weights = losses.bce_weight2(output_soft,args.num_classes)
        loss_ce = (CE(output, img_l, weights) * mask).sum() / (mask.sum() + 1e-16)
        loss_ce += (CE(output, patch_l) * patch_mask).sum() / (patch_mask.sum() + 1e-16)#loss = loss_ce
    elif mode == 2 and args.weightedbce:
        weights = losses.bce_weight2(output_soft,args.num_classes)
        loss_ce = (CE(output, img_l) * mask).sum() / (mask.sum() + 1e-16)
        loss_ce += (CE(output, patch_l, weights) * patch_mask).sum() / (
                    patch_mask.sum() + 1e-16)  # loss = loss_ce
    else:
        loss_ce = (CE(output, img_l) * mask).sum() / (mask.sum() + 1e-16)
        loss_ce += (CE(output, patch_l) * patch_mask).sum() / (patch_mask.sum() + 1e-16)  # loss = loss_ce
    ...
    for epoch in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            if epoch > args.pre_epoch:
                 logits, pcaW, loss_pca = model(net_input, W)
            else:
                 logits, pcaW, loss_pca = model(net_input)

            # logits_loss_dice, logits_loss_ce = mix_loss(logits, lab_a, lab_b, loss_mask, u_weight=1.0, unlab=True)
            logits_loss_dice, logits_loss_ce = mix_loss2(logits, lab_a, lab_b, loss_mask)

            # loss =  ((logits_loss_dice + logits_loss_ce) / 2)
            loss = (1-args.pca_weight) * ((logits_loss_dice + logits_loss_ce) / 2) + args.pca_weight * loss_pca
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            writer.add_scalar('info/mix_dice', logits_loss_dice, iter_num)
            writer.add_scalar('info/mix_ce', logits_loss_ce, iter_num)
            writer.add_scalar('info/loss_pca', loss_pca, iter_num)
            logging.info('iteration %d: loss: %f, mix_dice: %f, mix_ce: %f, loss_pca: %f'%(iter_num, loss, logits_loss_dice, logits_loss_ce, loss_pca))

            if iter_num % 20 == 0:
                image = net_input[1, 0:1, :, :]
                writer.add_image('pre_train/Mixed_Image', image, iter_num)
                outputs = torch.argmax(torch.softmax(logits, dim=1), dim=1, keepdim=True)
        ...
        writer.add_scalar('info/lr', cur_lr, epoch)
        if iter_num >= max_iterations:
            iterator.close()
            break
    writer.close()
...
def self_train(args ,pre_snapshot_path, snapshot_path):
    ...
    for epoch in iterator:
        for it, sampled_batch in enumerate(trainloader):
            ...
            unl_dice, unl_ce = mix_loss2(out_unl, plab_a, lab_a, loss_mask, mode=1)
            # l_dice, l_ce = mix_loss(out_l, lab_b, plab_b, loss_mask, u_weight=args.u_weight,mode=2)
            l_dice, l_ce = mix_loss2(out_l, lab_b, plab_b, loss_mask, mode=2)
...
if __name__ == "__main__":
    ...
    shutil.copy(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ACDC_BCP_train.py'), self_snapshot_path)
    #
    # Pre_train
