...
from utils.BCP_utils import context_mask, parameter_sharing, update_ema_variables
...
parser.add_argument('--exp', type=str,  default='CPP', help='exp_name')
parser.add_argument('--model', type=str, default='VNet', help='model_name')
parser.add_argument('--pre_max_iteration', type=int,  default=6000, help='maximum pre-train iteration to train') # 2000
parser.add_argument('--self_max_iteration', type=int,  default=20000, help='maximum self-train iteration to train')
parser.add_argument('--max_samples', type=int,  default=80, help='maximum samples to train')
parser.add_argument('--labeled_bs', type=int, default=4, help='batch_size of labeled data per gpu') # 4
parser.add_argument('--batch_size', type=int, default=8, help='batch_size per gpu') # 8
parser.add_argument('--base_lr', type=float,  default=0.015, help='maximum epoch number to train')
parser.add_argument('--deterministic', type=int,  default=1, help='whether use deterministic training')
parser.add_argument('--labelnum', type=int,  default=4, help='trained samples') # 4 8
...
parser.add_argument('--mask_ratio', type=float, default=1/2, help='ratio of mask/image') # 2/3
# -- setting of mixup
parser.add_argument('--u_alpha', type=float, default=2.0, help='unlabeled image ratio of mixuped image')
parser.add_argument('--loss_weight', type=float, default=0.5, help='loss weight of unimage term')
#flx
parser.add_argument('--weightedbce', type=bool, default=False, help='bce with weights') # 相对置信度函数，对训练的影响
parser.add_argument('--alpha', type=float, default=10, help='bce with weights') # 相对置信度函数参数
parser.add_argument('--beta', type=float, default=1.3, help='bce with weights') # 相对置信度函数参数
parser.add_argument('--BG', type=bool, default=False, help='Background')
...
parser.add_argument('--pca_weight', type=int, default=0.001, help='Background')
...
num_classes = 2
dice_loss = mask_DiceLoss(nclass=num_classes)
...
def pre_train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            if epoch_num > args.pre_epoch:
                 outputs, pcaW, loss_pca = model(volume_batch, W)
            else:
                 outputs, pcaW, loss_pca = model(volume_batch)
            ...
            if iter_num % 200 == 0:
                model.eval()
                dice_sample = test_3d_patch.var_all_case_LA(model, num_classes=num_classes, patch_size=patch_size, stride_xy=18, stride_z=4, root_path=args.root_path)
    ...
    for epoch in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                ...
                unoutput_a, pcaW_a, loss_pca_a = ema_model(unimg_a)
                unoutput_b, pcaW_a, loss_pca_b = ema_model(unimg_b)
            ...
            if epoch > args.self_epoch:
                outputs_l, pcaW_unl, loss_pca_unl = model(mixl_img, W)
                outputs_u, pcaW_l, loss_pca_l = model(mixu_img, W)
            else:
                outputs_l, pcaW_unl, loss_pca_unl = model(mixl_img)
                outputs_u, pcaW_l, loss_pca_l = model(mixu_img)

            l_dice, l_ce = mix_loss(outputs_l, lab_a, plab_a, loss_mask, u_weight=args.u_weight, unlab=True, mode=2)
            unl_dice, unl_ce = mix_loss(outputs_u, plab_b, lab_b, loss_mask, u_weight=args.u_weight, mode=1)
            ...
            writer.add_scalar('Self/loss_dice', loss_dice, iter_num)
            writer.add_scalar('Self/loss_all', loss, iter_num)

            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            logging.info('iteration %d : loss: %03f, loss_ce: %03f, loss_dice: %03f'%(iter_num, loss, loss_ce, loss_dice))
...
if __name__ == "__main__":
    ...
    shutil.copy(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'LA_BCP_train.py'), self_snapshot_path)
    # -- Pre-Training
